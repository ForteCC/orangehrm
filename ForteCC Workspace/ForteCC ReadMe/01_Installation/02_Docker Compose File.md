...
---
# Configure RDS on Project:
- The NGINX HTTPS needs to be turned OFF.
```bash
# Turn off the Nginx https
sudo systemctl stop nginx
```
- Once the Application is Installed Turn ON the nginx.
```bash
# Turn On the https
sudo systemctl start nginx

#If you also want it to automatically start on boot, run:
